...
\* Shared Memory Object: UFID object, whose state is represented as an 
\* idempotent function UF.rep and a mapping from element to id, UF.id.
\* ALGORITHM:

\*     0:  CHOOSE i, j \in [1..n]: Find_p(i) \/ Unite_p(i, j) \/ SameSet(i, j) \/ GetID(i)
...
CandID(sigma, x) == IF UF.rep[x] = x THEN {k \in Nat: k >= sigma.id[x]} ELSE {0}

IDUpdate(x) == {[UF.id EXCEPT ![x] = y]: y \in CandID(UF.rep, x)}

OpSet ==  [PROCESSES -> {"F", "U", "S", BOT}]
ArgSet == [PROCESSES -> {BOT} \cup NodeSet \cup NodeSet \X NodeSet]
ReturnSet == [PROCESSES -> {ACK, BOT, TRUE, FALSE} \cup NodeSet \cup Nat]

Configs == [sigma: Idems, op: OpSet, arg: ArgSet, ret: ReturnSet]


InitState == [i \in NodeSet |-> i]
InitUF ==     [rep |-> [i \in NodeSet |-> i], id |-> [i \in NodeSet |-> i]]
...
Init ==         /\ pc = [p \in PROCESSES |-> "0"]
                ...
                /\ ret \in [PROCESSES -> {ACK, TRUE, FALSE, 0, BOT} \cup NodeSet]
...
U1(p) == /\ pc[p] = "U1"
         ...
         /\ M' = {t \in Configs: \E told \in M:  /\ told.ret[p] = BOT
                                                 ...
                                                 /\ t.sigma = MetaConfigUnite(told.sigma, told.arg[1], told.arg[2])
                                                 /\ t.op = told.op
                                                 /\ t.arg = told.arg}
...
S2(p) == /\ pc[p] = "S2"
         /\ pc' = [pc EXCEPT ![p] = "S3"]
         /\ i_u' = [i_u EXCEPT ![p] = UF.id[u[p]]]
         /\ UNCHANGED <<UF, i_v, u, v, c, d, ret, M>>
...
S4(p) == /\ pc[p] = "S4"
         /\ i_v' = [i_v EXCEPT ![p] = UF.id[v[p]]]
         /\ UF'.rep = UF.rep
         /\ UF'.id \in IDUpdate(v[p])
...
\* Last modified Wed Feb 11 01:02:47 EST 2026 by karunram
