...
\*     U6:    else:
\*     U6:       if u < v then CAS(F[u], [a.parent, a.rank, 1], [v, a.rank+1, $])
\*     U6:       else: CAS(F[v], [b.parent, b.rank, 1], [u, b.rank+1, $])
\*     U7:    u = Find_p(u)
...
F2(p) ==        /\ IF F[u_F[p]].bit = 1
                ...
                /\ IF F[u_F[p]].bit = 1 /\ pc[p] = "F2"
                        THEN M' =   {t \in Configs: \E told \in M: /\ told.ret[p] = BOT 
                                                                    /\ t.sigma = told.sigma
...
F4(p) ==        /\ b_F' = [b_F EXCEPT ![p] = F[a_F[p].parent]]
                ...
                /\ IF F[a_F[p].parent].bit = 1 /\ pc[p] = "F4"
                        THEN M' = {t \in Configs: \E told \in M: /\ told.ret[p] = BOT 
                                                                  /\ t.sigma = told.sigma
                                                                  /\ t.ret = [told.ret EXCEPT ![p] = told.sigma[told.arg[p]]]
                                                                  /\ t.op = told.op
                                                                  /\ t.arg = told.arg}
                        ELSE M' = M
                /\ UNCHANGED <<F, u_F, a_F, u_U, v_U, a_U, b_U, c, d>>
...
F5(p) ==        /\ IF b_F[p].bit = 1
                ...
                /\ UNCHANGED <<F, a_F, b_F, u_F, u_U, v_U, a_U, b_U, c, d, M>>
...
FR(p) ==        /\ pc' = [pc EXCEPT ![p] = "0"]
                /\  \/ pc[p] = "FR"     /\ pc'  = [pc EXCEPT ![p] = "0"]
                                        /\ u_U' = u_U
                                        /\ M'   = {t \in Configs:   \E told \in M:  /\ told.ret[p] = u_F[p]
                                                                                    /\ t.sigma = told.sigma
                    ...
                    ELSE 
                            IF u_U[p] < v_U[p] \* ranks are equal
                                    THEN IF F[u_U[p]] = [parent |-> a_U[p].parent, rank |-> a_U[p].rank, bit |-> 1] \* u < v
                                            THEN    \/  /\ F' = [F EXCEPT ![u_U[p]] = [parent |-> v_U[p], rank |-> a_U[p].rank+1, bit |-> 0]]
                                                        /\ M' = {t \in Configs: \E told \in M:  \/  /\ told.ret[p] = BOT
                                                                                                    /\ t.ret = [told.ret EXCEPT ![p] = ACK]
                                                                                                    /\ t.sigma = [i \in NodeSet |-> IF told.sigma[i] = told.sigma[told.arg[p][1]] 
                                                                                                                                        THEN told.sigma[told.arg[p][2]] 
                                    ...
                                    ELSE IF u_U[p] > v_U[p] /\ F[v_U[p]] = [parent |-> b_U[p].parent, rank |-> b_U[p].rank, bit |-> 1] \* v > u
                                            THEN    \/  /\ F' = [F EXCEPT ![v_U[p]] = [parent |-> u_U[p], rank |-> b_U[p].rank+1, bit |-> 0]]
                                                        /\ M' = {t \in Configs: \E told \in M:  \/  /\ told.ret[p] = BOT
                                                                                                    /\ t.ret = [told.ret EXCEPT ![p] = ACK]
                                                                                                    /\ t.sigma = [i \in NodeSet |-> IF told.sigma[i] = told.sigma[told.arg[p][2]] 
                                                                                                                                        THEN told.sigma[told.arg[p][1]] 
...
UR(p) ==        /\ pc[p] = "UR"
                /\ pc' = [pc EXCEPT ![p] = "0"]
                /\ M' = {t \in Configs: \E told \in M: /\ told.ret[p] = ACK 
                                                        /\ t.sigma = told.sigma
                                                        /\ t.ret = [told.ret EXCEPT ![p] = BOT]
                                                        /\ t.op = [told.op EXCEPT ![p] = BOT]
                                                        /\ t.arg = [told.arg EXCEPT ![p] = BOT]}
...
\* Last modified Sun May 04 17:59:06 EDT 2025 by karunram
