...
F2(p) ==        /\ IF F[u_F[p]].bit = 1
                ...
                /\ IF F[u_F[p]].bit = 1 /\ pc[p] = "F2"
                        THEN M' =   {t \in Configs: \E told \in M: /\ told.ret[p] = BOT 
                                                                    /\ t.sigma = told.sigma
                                                                    /\ t.ret = [told.ret EXCEPT ![p] = told.sigma[t.arg[p]]]
                                                                    /\ t.op = told.op
                                                                    /\ t.arg = told.arg}
                        ELSE M' = M
                    ...
                    ELSE IF a_U[p].rank > b_U[p].rank
                            THEN IF F[v_U[p]] = [parent |-> b_U[p].parent, rank |-> b_U[p].rank, bit |-> 1]
                                    THEN    /\ F' = [F EXCEPT ![v_U[p]] = [parent |-> u_U[p], rank |-> b_U[p].rank, bit |-> 0]]
                                            /\ M' = {t \in Configs: \E told \in M:  \/  /\ told.ret[p] = BOT
                                                                                        /\ t.ret = [told.ret EXCEPT ![p] = ACK]
                                                                                        /\ t.sigma = [i \in NodeSet |-> IF told.sigma[i] = told.sigma[t.arg[p][2]] 
                                                                                                                        THEN told.sigma[t.arg[p][1]] 
                                                                                                                        ELSE told.sigma[i]]
                                                                                        /\ t.op = told.op
                                                                                        /\ t.arg = told.arg
                                                                                    \/  /\ told.ret[p] = ACK
                            ...
                            IF u_U[p] < v_U[p] \* ranks are equal
                                    THEN IF F[u_U[p]] = [parent |-> a_U[p].parent, rank |-> a_U[p].rank, bit |-> 1] \* u < v
                                            THEN    \/  /\ F' = [F EXCEPT ![u_U[p]] = [parent |-> v_U[p], rank |-> a_U[p].rank, bit |-> 0]]
                                                        /\ M' = {t \in Configs: \E told \in M:  \/  /\ told.ret[p] = BOT
                                                                                                    /\ t.ret = [told.ret EXCEPT ![p] = ACK]
                                                                                                    /\ t.sigma = [i \in NodeSet |-> IF told.sigma[i] = told.sigma[t.arg[p][1]] 
                                                                                                                                        THEN told.sigma[t.arg[p][2]] 
                                                                                                                                        ELSE told.sigma[i]]
                                                                                                    /\ t.op = told.op
                                                                                                    /\ t.arg = told.arg
                                                                                                \/  /\ told.ret[p] = ACK
...
\* Last modified Wed Apr 30 14:27:08 EDT 2025 by karunram
