...
THEOREM FunctionRespectsDeltaUF == 
  ...
  <1>1. delta_uf(s1, o, s2, r) => delta_uf_prime(f(s1), o, f(s2), r)
    <2> SUFFICES ASSUME delta_uf(s1, o, s2, r)
                 PROVE  delta_uf_prime(f(s1), o, f(s2), r)
      OBVIOUS
    <2>1. CASE /\ o[2] \in U
               /\ o[1] = "Find"
               /\ \E p \in s1 :
        ...
        <3> /\ o[1] = "Find"
            /\ r = f(s1)[o[2]]
            /\ f(s2) = f(s1)
            BY <2>1 DEF f, delta_uf_prime
        <3> QED
            BY <2>1, FunctionWellDefined DEF delta_uf_prime             
    <2>2. CASE /\ o[2] \in U /\ o[3] \in U
        ...
        <3>1. CASE s2 = (s1 \ {p, q}) \cup {[r |-> p.r, X |-> p.X \cup q.X]}
          <4> f(s2) = [i \in U |-> IF f(s1)[i] = f(s1)[o[3]]
            ...
            <5> QED
                BY <5>1, <5>2, <5>3
          <4> QED
            BY <2>2, FunctionWellDefined DEF delta_uf_prime
        ...
        <3> QED
            BY <3>1, <3>2, <2>2
    <2>3. QED
      BY <2>1, <2>2 DEF delta_uf, Ops_UF
  <1>2. ~delta_uf(s1, o, s2, r) => ~delta_uf_prime(f(s1), o, f(s2), r)
    <2> SUFFICES ASSUME ~delta_uf(s1, o, s2, r)
                 PROVE  ~delta_uf_prime(f(s1), o, f(s2), r)
      OBVIOUS
    <2>h. ~(\/ /\ o[1] = "Find"
    ...
    <2>1. CASE o[1] = "Find"
        ...
        <3>1. CASE \A p \in s1: o[2] \notin p.X \/ p.r # r
            <4> PICK p \in s1: o[2] \in p.X
                BY <2>1 DEF L_U, Ops_UF
            <4> QED
                BY MapsToRep, <2>1, <3>1 DEF delta_uf_prime, Ops_UF
        <3>2. CASE s2 # s1
            BY <2>1, <3>2, FunctionInjective DEF delta_uf_prime
        <3> QED
            BY <3>1, <3>2
    <2>2. CASE o[1] = "Unite"
        <3>a. \/ r # ACK
            ...
            BY <2>2, <2>h
         <3>1. CASE r # ACK
            BY <2>2, <3>1, <3>a DEF delta_uf_prime
         <3>2. CASE \A p1, p2 \in s1:
            ...
            <4>b. f(a2) = b2
                ...
                <5> QED
                    BY <5>1, <5>2, <5>3
            <4> QED
                BY <2>2, <4>k, <4>a, <4>b DEF delta_uf_prime, Ops_UF
        <3> QED
            BY <3>1, <3>2, <3>a
    <2> QED
        BY <2>1, <2>2
